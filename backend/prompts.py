...
## 核心原则：严格贴合原著
- 只提取小说中真实存在的角色，禁止编造或添加原著中没有的角色
- 角色姓名、性格、背景必须完全基于原文，不得虚构
- 若原文未明确某属性（如年龄），标注为 null，不要猜测

## 任务
从以下小说原文中提取所有角色信息，分析他们的性格、关系和角色弧光。
...
## 核心原则：严格贴合原著
- 对白内容必须基于小说原文中的对话和叙事，不得编造原著中不存在的对话
- 每个角色的说话风格必须与原文一致
- 情节走向必须与原文完全吻合，不得自行添加或删减剧情
- 覆盖原文中所有章节的主要情节，不得遗漏

## 任务
基于情节结构、角色信息和小说原文，为每一场生成节拍内容（对白、独白、旁白、动作）。
